...
func (s *ValidationTestSuite) assertCreateRepoHappy(t *check.C, doc svnman.CreateRepo) {
	result, err := ValidCreateRepo(&doc)
	if err != nil {
...
func (s *ValidationTestSuite) assertCreateRepoUnhappy(t *check.C, doc svnman.CreateRepo) {
	result, err := ValidCreateRepo(&doc)
	if err != nil {
		t.Errorf("validation of request failed completely: %s", err)
		return
